    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <div className="flex items-center gap-1.5 min-w-0">
          <span className={`text-sm font-semibold truncate ${isSelected ? 'text-white' : 'text-gray-900'}`}>{repo.name}</span>
          {repo.visibility === 'private' ? (
            <Lock size={11} className={isSelected ? 'text-gray-300 shrink-0' : 'text-gray-400 shrink-0'} />
          ) : (
            <Globe size={11} className={isSelected ? 'text-gray-300 shrink-0' : 'text-gray-400 shrink-0'} />
          )}
        </div>
      ...
      {/* Description */}
      <p className={`text-xs line-clamp-1 mb-2 ${isSelected ? 'text-gray-300' : 'text-gray-500'}`}>{repo.description}</p>

      {/* Language + stats */}
      <div className="flex items-center gap-3 mb-2">
        <span className={`flex items-center gap-1 text-xs ${isSelected ? 'text-gray-200' : 'text-gray-600'}`}>
          <span
        ...
        <span className={`flex items-center gap-1 text-xs ${isSelected ? 'text-gray-300' : 'text-gray-500'}`}>
