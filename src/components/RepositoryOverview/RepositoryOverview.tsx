...
function HealthRing({ score }: { score: number }) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <span className="absolute text-sm font-bold text-slate-100">{score}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-xl font-bold text-slate-100">{repo.name}</h1>
              <span className="flex items-center gap-1 text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full border border-slate-700">
                {repo.visibility === 'private' ? <Lock size={11} /> : <Globe size={11} />}
                {repo.visibility}
              </span>
            </div>
            <p className="text-sm text-slate-400 mb-3">{repo.fullName}</p>
            <p className="text-sm text-slate-400 max-w-xl">{repo.description}</p>
        ...
        {/* Last commit */}
        <div className="mt-3 px-3 py-2 bg-slate-800/50 rounded-lg border border-slate-700/50 text-xs text-slate-400 font-mono truncate">
          <span className="text-slate-600 mr-2">last commit</span>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-semibold text-slate-200">Run Agent Task</h2>
            <p className="text-xs text-slate-500 mt-0.5">Select an AI-powered task to run on this repository</p>
          </div>
          {isAgentRunning && (
          ...
          {agentTasks.map((task) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-start gap-2 p-3 rounded-lg border border-slate-700/50 bg-slate-800/40 hover:bg-slate-800 hover:border-blue-500/40 transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed group text-left"
              >
                <div className="p-1.5 rounded-md bg-blue-500/10 text-blue-400 group-hover:bg-blue-500/20 transition-colors">
                  <Icon size={15} />
                </div>
                <div>
                  <div className="text-xs font-medium text-slate-200">{task.label}</div>
                  <div className="text-xs text-slate-600 mt-0.5">{task.estimatedTime}</div>
                </div>
              </button>
            );
          })}
